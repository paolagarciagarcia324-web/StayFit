...
if (!function_exists('fpInsFormatearPeso')) {
    function fpInsFormatearPeso($peso) {
        if ($peso === null || $peso === '') {
            return '—';
        }
...
if (!function_exists('fpInsMedidasLista')) {
    function fpInsMedidasLista($item) {
        $mapa = [
            'cintura' => 'Cintura',
            'cadera' => 'Cadera',
            'brazos' => 'Brazo',
...
if (!function_exists('fpInsFormatearFechaProgreso')) {
    function fpInsFormatearFechaProgreso($fecha) {
        if ($fecha === null || $fecha === '') {
            return 'Sin fecha';
        }
...
foreach ($progresos as $reg) {
    if (!empty($reg['fotos_evolucion']) || !empty($reg['foto'])) {
        $conFoto++;
    }
}

$pesoMini = array_slice(array_reverse($progresos), 0, 6);
$pesosNumericos = array_values(array_filter(array_map(static function ($r) {
    return is_numeric($r['peso'] ?? null) ? (float) $r['peso'] : null;
}, $pesoMini), static function ($v) {
    return $v !== null;
}));
...
<head>
    ...
    <title>Progreso Institucional | StayFit</title>
...
<div class="cliente-wrapper">
    ...
    <main class="content">
        ...
        <section class="page-header">
            <h1>Progreso institucional</h1>
            <p>Registra tus avances individuales, aunque tu plan pertenezca a un convenio institucional.</p>
        </section>
        ...
        <main class="fp-content content">
            ...
            <section class="fp-hero hero page-header">
                ...
                <h1>Progreso <span>institucional</span></h1>
                <p>Registra tus avances individuales y revisa tu historial, aunque tu plan pertenezca a un convenio institucional.</p>
            </section>
            ...
            <section class="fp-stats-premium">
                <article class="fp-stat-premium fp-stat-premium--fuchsia">
                    ...
                    <p class="fp-stat-premium-value"><?= e(fpInsFormatearPeso($pesoActual)) ?></p>
            ...
            <section class="fp-progreso-trend">
                ...
                <div class="fp-progreso-trend-bars" role="img" aria-label="Gráfico de tendencia de peso">
                    <?php foreach ($pesosNumericos as $pesoBar): ?>
                        <?php $altura = 24 + (int) round((($pesoBar - $pesoMin) / $rangoPeso) * 56); ?>
                        <div class="fp-progreso-trend-bar" style="height: <?= e((string) $altura) ?>px;" title="<?= e(fpInsFormatearPeso($pesoBar)) ?>"></div>
                    <?php endforeach; ?>
                </div>
            </section>
            <?php endif; ?>
            ...
            <div class="fp-progreso-grid">
                <article class="fp-card card fp-progreso-card">
                    ...
                    <div class="fp-progreso-card-body">
                        <form class="fp-form-premium fp-progreso-form" action="../../controllers/clienteIns/progresoController.php?accion=registrar" method="POST" enctype="multipart/form-data">
                            <div class="fp-form-grid">
                                <div class="fp-field">
                                    <label for="prog-ins-peso">Peso actual (kg)</label>
                                    <input type="number" id="prog-ins-peso" step="0.1" min="20" max="300" name="peso" placeholder="Ej: 78.5" required>
                                </div>
                                <div class="fp-field fp-field--full">
                                    <label for="prog-ins-medidas">Medidas corporales</label>
                                    <textarea id="prog-ins-medidas" name="medidas" rows="2" placeholder="Ej: cintura 82 cm, cadera 98 cm, brazo 32 cm"></textarea>
                                    <span class="fp-field-hint">Puedes listar varias medidas en una sola línea.</span>
                                </div>
                                <div class="fp-field fp-field--full">
                                    <label for="prog-ins-observacion">Observación</label>
                                    <textarea id="prog-ins-observacion" name="observacion" rows="3" placeholder="Cuéntale a tu coach cómo te has sentido, tu energía o cambios que notes"></textarea>
                                </div>
                                <div class="fp-field fp-field--full">
                                    <label for="prog-ins-foto">Foto de progreso</label>
                                    <div class="fp-progreso-file">
                                        <input type="file" id="prog-ins-foto" name="foto" accept="image/*">
                                        <label for="prog-ins-foto" class="fp-progreso-file-label">
                        ...
                        <?php else: ?>
                            <div class="fp-progreso-timeline">
                                <?php foreach ($progresos as $index => $item): ?>
                                    <?php
                                    $medidas = fpInsMedidasLista($item);
                                    $foto = trim((string) ($item['fotos_evolucion'] ?? $item['foto'] ?? ''));
                                        ...
                                        <div class="fp-progreso-item-main">
                                            <header class="fp-progreso-item-head">
                                                <div>
                                                    ...
                                                    <strong class="fp-progreso-item-peso"><?= e(fpInsFormatearPeso($item['peso'] ?? null)) ?></strong>
                                                </div>
                                                <time class="fp-progreso-item-fecha" datetime="<?= e($item['fecha'] ?? '') ?>">
                                                    <?= e(fpInsFormatearFechaProgreso($item['fecha'] ?? '')) ?>
                                                </time>
                                            </header>
                                            ...
                                            <?php if ($foto !== ''): ?>
                                                <a class="fp-progreso-foto" href="../../<?= e(ltrim($foto, '/')) ?>" target="_blank" rel="noopener">
                                                    <img src="../../<?= e(ltrim($foto, '/')) ?>" alt="Foto de progreso del <?= e(fpInsFormatearFechaProgreso($item['fecha'] ?? '')) ?>">
                                                </a>
                                            <?php endif; ?>
                                        </div>
                                    </article>
...
<script>
(function () {
    var input = document.getElementById('prog-ins-foto');
